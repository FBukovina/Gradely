...
class AbsenceStateScreenInteractionTest {
    @get:Rule
    ...
    fun initialLoadingShowsLoadingCopyWithoutRetry() {
        ...
        setScreen(
            state = AbsencePresentationState.INITIAL_LOADING,
            errorMessage = null,
            onRetry = { retries.incrementAndGet() },
        )

        composeRule.onNodeWithText(context.getString(R.string.absence_loading)).assertIsDisplayed()
        composeRule.onNodeWithText(context.getString(R.string.absence_loading_subtitle)).assertIsDisplayed()
    ...
    fun firstLoadErrorShowsSuppliedMessageAndRetriesExactlyOnce() {
        ...
        composeRule.onNodeWithText(context.getString(R.string.absence_load_failed)).assertIsDisplayed()
    ...
    @Test
    fun firstLoadErrorWithoutMessageShowsFallbackAndRetriesExactlyOnce() {
        val retries = AtomicInteger(0)
        setScreen(
            state = AbsencePresentationState.FIRST_LOAD_ERROR,
            errorMessage = null,
        ...
        composeRule.onNodeWithText(context.getString(R.string.absence_load_failed)).assertIsDisplayed()
        composeRule.onNodeWithText(context.getString(R.string.absence_load_failed_subtitle)).assertIsDisplayed()
        assertRetryButtonAndClick()
        composeRule.runOnIdle { assertEquals(1, retries.get()) }
    }
    ...
    private fun retryNode() = composeRule.onNodeWithText(context.getString(R.string.absence_retry))
